...
const rollDice = function () {
  ...
  // Display dice
  dice.innerHTML = `<img class="dice" src="../assets/images/dice/dice-${randomNumber}.png" alt="dice ${randomNumber}">`;
...
const holdScore = function () {
  ...
  if (scores[activePlayer] >= 100) {
    document
      .querySelector(`.playerName-${activePlayer}`)
      .classList.add("winner-player");
    document.querySelector(
      `.playerName-${activePlayer}`
    ).innerHTML = `<p>winner !</p>`;
  } else {
    // Change player
    changePlayer();
  }
